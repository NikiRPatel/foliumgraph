...
zipcode_data = df.groupby(['Postcode'], as_index=False).agg({'Total SI': 'sum'}).rename(columns={'Total SI':'seriousInjury'})
zipcode_data.columns = ['postcode', 'seriousInjury']
zipcode_data['postcode'] = zipcode_data['postcode'].apply(str)
print(zipcode_data)
zipcode_data.to_csv("Serious_Injury.csv")
...
def map_feature_by_zipcode(zipcode_data, col):
    ...
    m = folium.Map(location=[138.6007,34.9285], zoom_start=9,
    ...
    m.choropleth(
        ...
        fill_opacity=0.9,
        line_opacity=0.2,
        legend_name="Number of Serious Injury in Road Crash"
    )
...
map_feature_by_zipcode(zipcode_data, 'seriousInjury')