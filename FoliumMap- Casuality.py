...
zipcode_data = df.groupby(['Postcode'], as_index=False).agg({'Total Cas': 'sum'}).rename(columns={'Total Cas':'Casualties'})
zipcode_data.columns = ['postcode', 'Casualties']
zipcode_data['postcode'] = zipcode_data['postcode'].apply(str)
print(zipcode_data)
zipcode_data.to_csv("Causality.csv")
...
def map_feature_by_zipcode(zipcode_data, col):
    ...
    m.choropleth(
        ...
        fill_opacity=0.9,
        line_opacity=0.2,
        legend_name="Number of Casualties in Road Crash"
    )
...
map_feature_by_zipcode(zipcode_data, 'Casualties')